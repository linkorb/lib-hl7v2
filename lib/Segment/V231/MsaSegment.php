...
class MsaSegment extends AbstractSegment
{
     ...
     * @var \Hl7v2\DataType\V231\IdDataType
     */
    private $delayedAcknowledgementType = null;
    /**
     ...
     * @param string $value
     */
    public function setFieldDelayedAcknowledgementType(string $value)
    {
        $this->delayedAcknowledgementType = $this
            ->dataTypeFactory
            ->create('ID', $this->encodingParameters)
        ;
        $this->delayedAcknowledgementType->setValue($value);
    }
     ...
     * @return \Hl7v2\DataType\V231\IdDataType
     */
    public function getFieldDelayedAcknowledgementType()
    {
        return $this->delayedAcknowledgementType;
    }
    ...
    public function fromDatagram(Datagram $datagram, Codec $codec)
    {
        ...
        $this->checkFieldLength('DelayedAcknowledgementType', 1, $datagram->getPositionalState());
        $this->setFieldDelayedAcknowledgementType($codec->extractComponent($datagram));

        // MSA.6
        if (false === $codec->advanceToField($datagram)) {
            return false;
        }
        $this->checkFieldLength('ErrorCondition', 100, $datagram->getPositionalState());
    ...
    public function __toString()
    {
        ...
        if (!$this->getFieldDelayedAcknowledgementType() || !$this->getFieldDelayedAcknowledgementType()->hasValue()) {
            ++$emptyFieldsSinceLastField;
        } else {
            $s .= str_repeat(
                $this->encodingParameters->getFieldSep(),
                1 + $emptyFieldsSinceLastField
            ) . (string) $this->getFieldDelayedAcknowledgementType()->getValue();
            $emptyFieldsSinceLastField = 0;
        }
