...
foreach ($finder->directories()->in(__DIR__ . '/data')->depth('== 0') as $versionedDir) {
    $types = Yaml::parse(file_get_contents($versionedDir->getPathname() . '/dataTypes.yml'));
    $subNamespace = 'V' . str_replace('.', '', $versionedDir->getBasename());
    $outDir = __DIR__ . '/../lib/DataType/' . $subNamespace;
    Util::mkOutDir($outDir);
    $context = new DataTypeContext('Hl7v2\\DataType', $subNamespace);
    $typeResolver = new DataTypeResolver($context, $types);

    foreach ($typeResolver->reseolveTypeDependencyGraph() as $typeId) {
        if (!isset($types[$typeId])) {
            echo 'No metadata for ' . $versionedDir->getBasename() . ' type ' . $typeId . \PHP_EOL;
            exit;
        }
